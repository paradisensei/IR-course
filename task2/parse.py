...
for a in articles:
    ...
    title = a.text
    title_porter = ' '.join(map(Porter.stem, title.split()));
    title_mystem = ''.join(m.lemmatize(title))
    
    abstract = article.xpath(u"//b[contains(text(), 'Аннотация')]/following-sibling::text()")[0].strip()
    abstract_porter = ' '.join(map(Porter.stem, abstract.split()));
    abstract_mystem = ''.join(m.lemmatize(abstract))
